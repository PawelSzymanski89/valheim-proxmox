...
if [ -f "$0" ] && [ -d "$(dirname "$0")/panel" ]; then
  cp "$(dirname "$0")"/panel/{app.py,index.html,login.html,icon.svg,greetings.json} "$VH_DIR/panel/"
else
  for f in app.py index.html login.html icon.svg greetings.json; do curl -fsSL "$REPO_RAW/panel/$f" -o "$VH_DIR/panel/$f"; done
fi
